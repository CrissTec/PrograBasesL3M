...
app.controller ("registroController", function($scope, $http) {
	...
	$scope.click = function () {
	    if ($scope.User.usuario == "") {
	        ...
	        $http.post('http://localhost:9820/api/LogIn?ced1=' + $scope.User.ced1 + '&ced2=' + $scope.User.ced2 + '&ced3=' + $scope.User.ced3 + '&nom=' + $scope.User.nombre + '&apell1=' + $scope.User.apellido1 + '&apell2=' + $scope.User.apellido2 + '&fecha=' + $scope.User.fecha + '&sex=' + $scope.User.genero + '&username=' + $scope.User.usuario + '&password=' + $scope.User.contrasena)
                .success(function (data, status, headers, config) {
                    $scope.exito = data;
                    if (data == "true") {
                        localStorage.setItem("USERNAME", $scope.User.username);
                        localStorage.setItem("CEDULA", $scope.User.Cedula);
